				...
				if(announce && i == 0){
					...
					if(randopt->msg_id && battle_config.refine_option_announce){
						...
						if(option->is_rare_announce){
							sprintf(rare_msg,msg_txt(NULL,2304),opt_msg);
						}else{
							sprintf(rare_msg,msg_txt(NULL,2303),opt_msg);
						}
						...
						sprintf(boardcast_msg,msg_txt(sd,2301),char_name,id->ename.c_str(),item.refine,rare_msg);
						...
						sprintf(self_msg,msg_txt(NULL,2305),id->ename.c_str(),item.refine,opt_msg);
