...
int npc_touch_areanpc(map_session_data* sd, int16 m, int16 x, int16 y, struct npc_data *nd)
{
	...
	case NPCTYPE_WARP:
		...
		if (sd->sc.option & OPTION_AUTOATTACK) {
			if (m != map_mapindex2mapid(nd->u.warp.mapindex)) { //same map ok to warp
				break;
			}
		}
	...
	case NPCTYPE_SCRIPT:
		...
		if (npc_ontouch_event(sd, nd) > 0 && npc_ontouch2_event(sd, nd) > 0 && !(sd->sc.option & OPTION_AUTOATTACK)) { // failed to run OnTouch event, so just click the npc
			if (!util::vector_exists(sd->areanpc, nd->bl.id))
				sd->areanpc.push_back(nd->bl.id);
			if (sd->npc_id == 0)
				npc_click(sd, nd);
...
e_purchase_result npc_barter_purchase(map_session_data & sd, std::shared_ptr<s_npc_barter> barter, std::vector<s_barter_purchase>&purchases, int16 index) {
	...
	for( s_barter_purchase& purchase : purchases ){
		...
		if (sd.state.craft_barter){
			...
			bonus_chance += sd.bonus.barter_craft_bonus;
			if(sd.sc.getSCE(SC_BARTER_CRAFT_BONUS))
				bonus_chance += sd.sc.getSCE(SC_BARTER_CRAFT_BONUS)->val1;

			if(rnd()%10000<(purchase.item->craft_rate+bonus_chance))
				success++;
			else
				fail++;
			...
			if(success){
				...
				sprintf(ann_msg,msg_txt(NULL,3010),item_db.create_item_link(it).c_str());
				...
				if(purchase.item->announce){
					sprintf(ann_msg, msg_txt(NULL, 3011),sd.status.name,item_db.create_item_link(it).c_str());
			...
			if(fail){
				sprintf(ann_msg,msg_txt(NULL,3012),item_db.create_item_link(it).c_str());
				clif_messagecolor(&sd.bl, color_table[COLOR_CYAN], ann_msg, false, SELF);
