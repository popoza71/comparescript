...
TIMER_FUNC(mob_delayspawn){
	...
	*/
	if(md->state.boss && battle_config.pvp_mvp_map) {
...
int mob_spawn (struct mob_data *md)
{
	...
	if (md->spawn) { //Respawn data
		...
		MF_MAPMVP - Disable going to savepoint once you die 2 times in a PVP map.
		*/
		if(md->spawn->state.boss && battle_config.pvp_mvp_map) {
...
int mob_getdroprate(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, mob_data * md) //gum spilt

{
	...
	std::shared_ptr<item_data> id = item_db.find(nameid); //gum spilt
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			// PUPPY EXTRA BUFF
			if (sd->sc.getSCE(SC_BONUSDROP))
				drop_rate_bonus += sd->sc.getSCE(SC_BONUSDROP)->val1;
			if (sd->sc.getSCE(SC_BONUSDROP2))
				drop_rate_bonus += sd->sc.getSCE(SC_BONUSDROP2)->val1;
...
DropRateResult mob_getdroprate_extended(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, mob_data* md)
{
	int drop_rate = base_rate;
	int bonus_drop_rate = 0;
	...
	if (src) {
		...
		if (src->type == BL_PC) {
			...
			bonus_drop_rate += base_rate * (sd->indexed_bonus.dropaddclass[mob->status.class_] + sd->indexed_bonus.dropaddclass[CLASS_ALL]) / 100;
			bonus_drop_rate += base_rate * (sd->indexed_bonus.dropaddrace[mob->status.race] + sd->indexed_bonus.dropaddrace[RC_ALL]) / 100;

			if (sd->sc.getSCE(SC_ITEMBOOST))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_ITEMBOOST)->val1 / 100;

			if (sd->sc.getSCE(SC_AID_PERIOD_RECEIVEITEM_2ND))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_AID_PERIOD_RECEIVEITEM_2ND)->val1 / 100;

			if (sd->sc.getSCE(SC_AID_PERIOD_RECEIVEITEM))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_AID_PERIOD_RECEIVEITEM)->val1 / 100;

			if (sd->sc.getSCE(SC_BONUSDROP))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_BONUSDROP)->val1 / 100;
			if (sd->sc.getSCE(SC_BONUSDROP2))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_BONUSDROP2)->val1 / 100;

			if (sd->sc.getSCE(SC_GUM_ETC) && (id->type == IT_USABLE || id->type == IT_HEALING || id->type == IT_ETC))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_GUM_ETC)->val1 / 100;

			if (sd->sc.getSCE(SC_GUM_EQUIP) && (id->type == IT_ARMOR || id->type == IT_WEAPON))
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_GUM_EQUIP)->val1 / 100;

			if (sd->sc.getSCE(SC_GUM_CARD) && id->type == IT_CARD)
				bonus_drop_rate += base_rate * sd->sc.getSCE(SC_GUM_CARD)->val1 / 100;
	...
	int total_drop_rate = base_rate + bonus_drop_rate;
	if (total_drop_rate > battle_config.drop_rate_cap && base_rate < battle_config.drop_rate_cap) {
		total_drop_rate = battle_config.drop_rate_cap;
	}

	DropRateResult result = { base_rate, bonus_drop_rate, total_drop_rate };
	return result;
}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			drop_rate = mob_getdroprate(src, md->db, md->db->dropitem[i].rate, drop_modifier, md->db->dropitem[i].nameid, md); //gum spilt
