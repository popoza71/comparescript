...
ACMD_FUNC(kami)
{
	...
	if(*(command + 5) != 'c' && *(command + 5) != 'C') {
		...
		if (strstr(command, "l") != NULL){
			for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
				if (pl_sd->bl.m == sd->bl.m && battle_config.broadcast_sound)
		...
		//else
			...
			for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
				if (battle_config.broadcast_sound)
				clif_soundeffect( pl_sd->bl, "anuncio.wav", 0, SELF );
...
ACMD_FUNC(mobinfo)
{
	...
	for (uint16 k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			//int droprate = mob_getdroprate(&sd->bl, mob, mob->dropitem[i].rate, drop_modifier, mob->dropitem[i].nameid); //gum spilt
			//
			//sprintf(atcmd_output2, " - %s  %02.02f%%", item_db.create_item_link(id).c_str(), (float)droprate / 100); 


			//puppy @mi
			DropRateResult droprate = mob_getdroprate_extended(&sd->bl, mob, mob->dropitem[i].rate, drop_modifier, mob->dropitem[i].nameid);

			sprintf(atcmd_output2, " - %s  (%02.02f%%) + (%02.02f%%) = %02.02f%%",
				item_db.create_item_link(id).c_str(),
				(float)droprate.normal_drop_rate / 100,
				(float)droprate.bonus_drop_rate / 100,
...
// ฟังก์ชันสำหรับล้างการตั้งค่า autopots ของผู้เล่น
void autoatpots_clean(struct map_session_data* sd)
{
	if (sd) {
		sd->sc.option &= ~OPTION_AUTOPOTS; // ปิดการใช้งาน autopots
		sd->autopots.hp_nameid = 0;
		sd->autopots.hp_rate = 0;
		sd->autopots.sp_nameid = 0;
		sd->autopots.sp_rate = 0;
		sd->autopots.hp_rate_max = 0;
		sd->autopots.sp_rate_max = 0;
		sd->autopots.delay_rate = 0;
		sd->autopots.hp_auto = 0;
		sd->autopots.sp_auto = 0;
		clif_changeoption(&sd->bl); // ส่งข้อมูลการเปลี่ยนแปลงไปยังผู้เล่น
	}
}

// ฟังก์ชัน timer สำหรับจัดการกับ autopots
static TIMER_FUNC(autoatpots_timer)
{
	struct map_session_data* sd = map_id2sd(id);
	if (!sd) return 0;

	if (sd->sc.option & OPTION_AUTOPOTS) {
		int my_hp = (sd->battle_status.hp * 100) / sd->battle_status.max_hp;
		int my_sp = (sd->battle_status.sp * 100) / sd->battle_status.max_sp;
		int index;

		if (!pc_isdead(sd)) {
			if (!sd->autopots.sp_rate && !sd->autopots.hp_rate) {
				clif_displaymessage(sd->fd, msg_txt(sd, 2501));
				autoatpots_clean(sd);
				return 0;
			}

			// จัดการ HP autopots
			if (sd->autopots.hp_auto) {
				if (my_hp < sd->autopots.hp_rate_max && sd->autopots.hp_nameid && sd->autopots.hp_rate_max) {
					ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == sd->autopots.hp_nameid);
					if (sd->inventory.u.items_inventory[index].nameid == sd->autopots.hp_nameid) {
						sd->canuseitem_tick -= 200;
						pc_useitem(sd, index);
					}
				}
				else {
				...
				if (my_hp < sd->autopots.hp_rate && sd->autopots.hp_nameid && sd->autopots.hp_rate) {
					ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == sd->autopots.hp_nameid);
					if (sd->inventory.u.items_inventory[index].nameid == sd->autopots.hp_nameid) {
						sd->canuseitem_tick -= 200;
						sd->autopots.hp_auto = 1;
						pc_useitem(sd, index);
					}
				}
			}
			...
			// จัดการ SP autopots
			if (sd->autopots.sp_auto) {
				if (my_sp < sd->autopots.sp_rate_max && sd->autopots.sp_nameid && sd->autopots.sp_rate_max) {
					ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == sd->autopots.sp_nameid);
					if (sd->inventory.u.items_inventory[index].nameid == sd->autopots.sp_nameid) {
						sd->canuseitem_tick -= 200;
						pc_useitem(sd, index);
					}
				}
				...
				if (my_sp < sd->autopots.sp_rate && sd->autopots.sp_nameid && sd->autopots.sp_rate) {
					ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == sd->autopots.sp_nameid);
					if (sd->inventory.u.items_inventory[index].nameid == sd->autopots.sp_nameid) {
						sd->canuseitem_tick -= 200;
						sd->autopots.sp_auto = 1;
						pc_useitem(sd, index);
					}
				}
			}
		}
		add_timer(gettick() + sd->autopots.delay_rate, autoatpots_timer, sd->bl.id, 0); // ตั้งค่า timer ใหม่
	}
	return 0;
}
...
ACMD_FUNC(autopots)
{
	int hp_rate = 0, hp_nameid = 0, sp_rate = 0, sp_nameid = 0, type = 0, delay_rate = 0, hp_rate_ = 0, sp_rate_ = 0;
	nullpo_retr(-1, sd);

	if (!message || !*message ||
		sscanf(message, "%d %d %d %d %d %d %d %d", &type, &hp_rate, &hp_nameid, &sp_rate, &sp_nameid, &delay_rate, &hp_rate_, &sp_rate_) < 8 ||
		(type <= 0 || type > 1))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, 2501));
		autoatpots_clean(sd);
	...
	// จำกัดค่าให้อยู่ในขอบเขตที่กำหนด
	if (hp_rate > 99) hp_rate = 99;
	if (hp_rate < 1) hp_rate = 1;
	if (sp_rate > 99) sp_rate = 99;
	if (sp_rate < 1) sp_rate = 1;
	if (hp_rate > hp_rate_) hp_rate_ = hp_rate;
	if (sp_rate > sp_rate_) sp_rate_ = sp_rate;
	...
	if (!item_db.exists(hp_nameid) && hp_nameid) {
		hp_nameid = 0;
		hp_rate = 0;
		clif_displaymessage(sd->fd, msg_txt(sd, 2502));
	}
	if (!item_db.exists(sp_nameid) && sp_nameid) {
		sp_nameid = 0;
		sp_rate = 0;
		clif_displaymessage(sd->fd, msg_txt(sd, 2503));
	}

	if (delay_rate < 50) delay_rate = 50;

	clif_displaymessage(sd->fd, msg_txt(sd, 2504));
	...
	sd->autopots.hp_rate_max = hp_rate_;
	sd->autopots.sp_rate_max = sp_rate_;
	sd->autopots.delay_rate = delay_rate;
	sd->autopots.hp_auto = 0;
	sd->autopots.sp_auto = 0;

	add_timer(gettick() + delay_rate, autoatpots_timer, sd->bl.id, 0);
	clif_changeoption(&sd->bl);
...
bool autoattack_wing_exist(struct map_session_data* sd) {

	int index = -1;
	ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == battle_config.autoattack_wing_itemid);
...
static int buildin_autoattack_sub(struct block_list* bl, va_list ap)
{
	...
	if (battle_check_range(src, bl, range)) {
		if (sd->autoattack_mob_id > 0) {
			if (md->mob_id == sd->autoattack_mob_id) {
				*target_id = bl->id;
				return 1;
			}
		}
	...
	else if (unit_can_reach_pos(src, bl->x, bl->y, 0) && path_search_long(NULL, m, x, y, bl->x, bl->y, CELL_CHKWALL) && path_search_long(NULL, m, x, y, bl->x, bl->y, CELL_CHKNOPASS)) {
		if (sd->autoattack_mob_id > 0) {
			if (md->mob_id == sd->autoattack_mob_id) {
				*target_id = bl->id;
				return 1;
...
int autoattack_pickupitem(struct block_list* bl, va_list ap)
{
	...
	if (src->type == BL_PC && (sd = (struct map_session_data*)src) && bl->type == BL_ITEM && (fitem = (struct flooritem_data*)bl)) {
		...
		if (check_distance_bl(&sd->bl, &fitem->bl, 1) && (fitem->first_get_charid == sd->status.char_id || fitem->second_get_charid == sd->status.char_id || fitem->third_get_charid == sd->status.char_id)) {
			...
			if (allow_to_pick) {
				if (pc_takeitem(sd, fitem)) {
					//sd->autoattack_pickuptime = gettick()+battle_config.autoattack_interval;
					sd->autoattack_fitem_x = 0;
					sd->autoattack_fitem_y = 0;
					return 1;
				}
			}
		}

		if (unit_can_reach_pos(src, fitem->bl.x, fitem->bl.y, 0) && path_search_long(NULL, sd->bl.m, sd->bl.x, sd->bl.y, fitem->bl.x, fitem->bl.y, CELL_CHKWALL) && path_search_long(NULL, sd->bl.m, sd->bl.x, sd->bl.y, fitem->bl.x, fitem->bl.y, CELL_CHKNOPASS) && (fitem->first_get_charid == sd->status.char_id || fitem->second_get_charid == sd->status.char_id || fitem->third_get_charid == sd->status.char_id)) {
			if (sd->autoattack_fitem_x < 1 && sd->autoattack_fitem_y < 1) {
				sd->autoattack_fitem_x = fitem->bl.x;
				sd->autoattack_fitem_y = fitem->bl.y;
			}
			return 0;
		}
	}
...
static int buildin_autoattack_atker_sub(struct block_list* bl, va_list ap)
{
	int* target_id = va_arg(ap, int*);
	int16 m = va_arg(ap, int);
	int16 x = va_arg(ap, int);
	...
	current_target_id = battle_gettarget(bl);

	if (path_search_long(NULL, m, x, y, bl->x, bl->y, CELL_CHKWALL) && current_target_id == src->id && current_target_id > 0) {
		*target_id = bl->id;
	}
	else {
		struct status_data* tstatus = status_get_status_data(bl);
		if (status_has_mode(tstatus, MD_AGGRESSIVE) && status_has_mode(tstatus, MD_CANMOVE) && status_has_mode(tstatus, MD_CANATTACK))
			*target_id = bl->id;
	}
	return 1;
}
...
static int check_monster_in_cell(struct block_list* bl, va_list ap) {
	bool* is_cell_occupied = va_arg(ap, bool*);
...
void autoattack_motion(struct map_session_data* sd, int mob_id) {
	int i, target_id, item_count, x, y, mob_count, index;
	...
	if (pc_isdead(sd) && (sd->sc.option & OPTION_AUTOATTACK)) {
		sd->sc.option &= ~OPTION_AUTOATTACK;
		clif_displaymessage(sd->fd, msg_txt(sd, 2505));
		pc_setpos(sd, mapindex, sd->status.save_point.x, sd->status.save_point.y, CLR_OUTSIGHT);
		clif_displaymessage(sd->fd, msg_txt(sd, 7)); // Warping to save point..
		return;
	}

	if (pc_is90overweight(sd) && (sd->sc.option & OPTION_AUTOATTACK)) {
		sd->sc.option &= ~OPTION_AUTOATTACK;
	...
	if (sd->autoattack_force_wing && autoattack_wing_exist(sd)) {
		ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == battle_config.autoattack_wing_itemid);
		if (index < MAX_INVENTORY) {
			sd->autoattack_force_wing = false;
			sd->autoattack_can_sit = gettick() + 1500;
			pc_useitem(sd, index);
			return;
		}
	}
	...
	status = status_get_status_data(&sd->bl);
	src = map_id2bl(sd->bl.id);

	if (sd->autoattack_need_rest) {
		if (pc_is50overweight(sd)) {
			sd->autoattack_need_rest = false;
			return;
		}

		// เปลี่ยนการตรวจสอบ HP ให้ออกจากการพักเป็นการใช้ค่าจาก battle_config
		if (status->hp >= (status->max_hp * battle_config.autoattack_out_rest / 100)) {
			if (pc_issit(sd)) {
				pc_setstand(sd, true);
			}
			sd->autoattack_need_rest = false;
		...
		target_id = 0;
		for (i = 1; i <= 8; i++) {
			map_foreachinarea(buildin_autoattack_atker_sub, src->m, sd->bl.x - i, sd->bl.y - i, sd->bl.x + i, sd->bl.y + i, BL_MOB, &target_id, src->m, sd->bl.x, sd->bl.y, src);
			if (target_id > 0) {
				if (pc_issit(sd)) {
					pc_setstand(sd, true);
					if (autoattack_wing_exist(sd)) {
						sd->autoattack_force_wing = true;
					}
					return;
				}
				else {
				...
				if (!pc_issit(sd) && sd->autoattack_can_sit <= gettick()) {
					pc_setsit(sd);
					skill_sit(sd, 1);
					clif_sitting(&sd->bl);
					return;
	...
	// เปลี่ยนการตรวจสอบ HP ต่ำกว่าเป็นการใช้ค่าจาก battle_config
	if (status->hp < (status->max_hp * battle_config.autoattack_wing_hp_min / 100) && !pc_is50overweight(sd) && autoattack_wing_exist(sd)) {
		if (!sd->autoattack_need_rest) {
			sd->autoattack_need_rest = true;
			return;
		}
	}

	// เก็บไอเท็มก่อน
	if (battle_config.pickitem_first) {
		for (i = 0; i <= 14; i++) {
			item_count = map_foreachinrange(autoattack_pickupitem, &sd->bl, i, BL_ITEM, &sd->bl);
			if (item_count) {
				sd->autoattack_wing_time = 0;
				return;
			}
			if (!item_count && sd->autoattack_fitem_x > 0 && sd->autoattack_fitem_y > 0) {
				x = sd->autoattack_fitem_x;
				y = sd->autoattack_fitem_y;

				// Check if the target cell is occupied by a monster
				bool is_cell_occupied = false;
				map_foreachincell(check_monster_in_cell, sd->bl.m, x, y, BL_MOB, &is_cell_occupied);
				if (is_cell_occupied) {
					x = sd->autoattack_fitem_x;
					y = sd->autoattack_fitem_y;
				}

				if (!path_search(NULL, sd->bl.m, sd->bl.x, sd->bl.y, x, y, 0, CELL_CHKNOREACH)) {
					x = sd->autoattack_fitem_x;
					y = sd->autoattack_fitem_y;
				}
	...
	if (!sd->autoattack_need_rest) {
		...
		for (i = 1; i <= 14; i++) {
			target_id = 0;
			mob_count = map_foreachinarea(buildin_autoattack_sub, src->m, sd->bl.x - i, sd->bl.y - i, sd->bl.x + i, sd->bl.y + i, BL_MOB, &target_id, src->m, sd->bl.x, sd->bl.y, src);
			if (target_id) {
				target = map_id2bl(target_id);
				int range = status_get_range(src);
				if (!battle_check_range(src, target, range)) {
					unit_walktobl(&sd->bl, target, range, 2);
				}
				else {
					if (target_id && mob_count > 1) {
						target = map_id2bl(target_id);
						if (battle_gettarget(target) && battle_gettarget(target) != sd->bl.id) {
							target_id = 0;
							target = NULL;
							return;
						}
	...
	// ถ้าเลือกให้โจมตีก่อนแล้วถึงค่อยเก็บไอเท็ม
	if (!battle_config.pickitem_first) {
		for (i = 0; i <= 14; i++) {
			item_count = map_foreachinrange(autoattack_pickupitem, &sd->bl, i, BL_ITEM, &sd->bl);
			if (item_count) {
			...
			if (!item_count && sd->autoattack_fitem_x > 0 && sd->autoattack_fitem_y > 0) {
				x = sd->autoattack_fitem_x;
				y = sd->autoattack_fitem_y;
				if (abs(sd->bl.x - sd->autoattack_fitem_x) < 2 && abs(sd->bl.y - sd->autoattack_fitem_y) < 2) {
					sd->autoattack_fitem_x = 0;
					sd->autoattack_fitem_y = 0;
					return;
				}
				if (sd->bl.x < sd->autoattack_fitem_x - 1)
					x--;
				if (sd->bl.y < sd->autoattack_fitem_y - 1)
					y--;
				if (sd->bl.x > sd->autoattack_fitem_x + 1)
					x++;
				if (sd->bl.y > sd->autoattack_fitem_y + 1)
					y++;
				if (abs(sd->bl.x - sd->autoattack_fitem_x) == 1)
					x = sd->bl.x;
				if (abs(sd->bl.y - sd->autoattack_fitem_y) == 1)
					y = sd->bl.y;
				if (!path_search(NULL, sd->bl.m, sd->bl.x, sd->bl.y, x, y, 0, CELL_CHKNOREACH)) {
					x = sd->autoattack_fitem_x;
					y = sd->autoattack_fitem_y;
	...
	// การเดิน
	if (!target_id && !unit_is_walking(&sd->bl) && !sd->autoattack_need_rest) {
		int tx, ty, k;
		bool can_walk = false;
		if (sd->autoattack_dir_x == 0 && sd->autoattack_dir_y == 0) {
			sd->autoattack_dir_x = (rnd() % 2) ? -1 : 1;
			sd->autoattack_dir_y = (rnd() % 2) ? -1 : 1;
		}
		for (k = 0; k < 7; k++) {
			tx = sd->bl.x + (sd->autoattack_dir_x * ((rand() % 13) + 1));
			ty = sd->bl.y + (sd->autoattack_dir_y * ((rand() % 13) + 1));
			if (rnd() % 2) {
				if (rnd() % 3 == 0) {
					tx = sd->bl.x;
				}
			...
			if (unit_can_reach_pos(&sd->bl, tx, ty, 1)) {
				can_walk = true;
				break;
			}
		}
		if (can_walk) {
			unit_walktoxy(&sd->bl, tx, ty, 8);
		}
		else {
			sd->autoattack_dir_x = 0;
			sd->autoattack_dir_y = 0;
		}
	}

	// การใช้วิง
	sd->autoattack_wing_time += battle_config.autoattack_interval;
	if (battle_config.autoattack_wing_time && battle_config.autoattack_wing_itemid && sd->autoattack_wing_time >= battle_config.autoattack_wing_time) {
		ARR_FIND(0, MAX_INVENTORY, index, sd->inventory.u.items_inventory[index].nameid == battle_config.autoattack_wing_itemid);
		if (index < MAX_INVENTORY) {
			sd->autoattack_wing_time = 0;
			pc_useitem(sd, index);
			...
			sd->autoattack_wing_time = battle_config.autoattack_wing_time;
		}
	}
}


void autoattack_buff_self(struct map_session_data* sd)
{
	// ตรวจสอบว่ามียาเร่งก่อน
	int index = -1;
	ARR_FIND(0, MAX_INVENTORY, index,
		sd->inventory.u.items_inventory[index].nameid == 657 ||
		sd->inventory.u.items_inventory[index].nameid == 656 ||
		sd->inventory.u.items_inventory[index].nameid == 645);
	if (index < MAX_INVENTORY) {
		// ไม่มีบัฟยาเร่งก่อนหน้านี้
		if (!sd->sc.getSCE(SC_ASPDPOTION0) &&
			!sd->sc.getSCE(SC_ASPDPOTION1) &&
			!sd->sc.getSCE(SC_ASPDPOTION2)) {
			// ใช้ยาเร่ง
			if (sd->inventory.u.items_inventory[index].nameid == 657) {
				pc_useitem(sd, index);
				return;
			}
			else if (sd->inventory.u.items_inventory[index].nameid == 656) {
				pc_useitem(sd, index);
				return;
			}
			else if (sd->inventory.u.items_inventory[index].nameid == 645) {
				pc_useitem(sd, index);
				return;
			}
		}
	...
	// ตรวจสอบว่ามี Bubble Gum
	index = -1;
	ARR_FIND(0, MAX_INVENTORY, index,
		sd->inventory.u.items_inventory[index].nameid == 40505 ||
		sd->inventory.u.items_inventory[index].nameid == 40501 ||
		sd->inventory.u.items_inventory[index].nameid == 40504 ||
		sd->inventory.u.items_inventory[index].nameid == 40500);
	if (index < MAX_INVENTORY) {
		// ไม่มีบัฟ Bubble Gum ก่อนหน้านี้
		if (!sd->sc.getSCE(SC_ITEMBOOST)) {
			// ใช้ Bubble Gum
			if (sd->inventory.u.items_inventory[index].nameid == 40505) {
				pc_useitem(sd, index);
				return;
			}
			else if (sd->inventory.u.items_inventory[index].nameid == 40501) {
				pc_useitem(sd, index);
				return;
			}
			else if (sd->inventory.u.items_inventory[index].nameid == 40504) {
				pc_useitem(sd, index);
				return;
			}
			else if (sd->inventory.u.items_inventory[index].nameid == 40500) {
				pc_useitem(sd, index);
				return;
			}
		}
	...
	//check if sp <50% dont use skill buff
	if (sd->battle_status.sp < (sd->battle_status.max_sp / 2)) {
		clif_displaymessage(sd->fd, msg_txt(sd, 2507));
		return;
	}
	...
	// Acolyte Class
	/////////////////////////////////////
	if (sd->class_ == MAPID_ACOLYTE || sd->class_ == MAPID_PRIEST || sd->class_ == MAPID_MONK) {
		// Increase AGI
		int agi_lv = pc_checkskill(sd, AL_INCAGI);
		if (agi_lv > 0 && !sd->sc.getSCE(SC_INCREASEAGI)) {
			skill_castend_nodamage_id(&sd->bl, &sd->bl, AL_INCAGI, agi_lv, gettick(), 0);
			//use sp cost for use skill
			int sp_cost = skill_get_sp(AL_INCAGI, agi_lv);
			status_zap(&sd->bl, 0, sp_cost);
		}
		// Blessing
		int blessing_lv = pc_checkskill(sd, AL_BLESSING);
		if (blessing_lv > 0 && !sd->sc.getSCE(SC_BLESSING)) {
			skill_castend_nodamage_id(&sd->bl, &sd->bl, AL_BLESSING, blessing_lv, gettick(), 0);
			//use sp cost for use skill 
			int sp_cost = skill_get_sp(AL_BLESSING, blessing_lv);
			status_zap(&sd->bl, 0, sp_cost);
		}
		//Magnificat
		int magnificat_lv = pc_checkskill(sd, PR_MAGNIFICAT);
		if (magnificat_lv > 0 && !sd->sc.getSCE(SC_MAGNIFICAT)) {
			skill_castend_nodamage_id(&sd->bl, &sd->bl, PR_MAGNIFICAT, magnificat_lv, gettick(), 0);
			//use sp cost for use skill
			int sp_cost = skill_get_sp(PR_MAGNIFICAT, magnificat_lv);
			status_zap(&sd->bl, 0, sp_cost);
		}
	}
	/////////////////////////////////////
	// Swordman Class
	/////////////////////////////////////
	if (sd->class_ == MAPID_KNIGHT) {
		//Twohand Quicken
		if (sd && sd->status.weapon == W_2HSWORD) {
			int twohandquicken_lv = pc_checkskill(sd, KN_TWOHANDQUICKEN);
			if (twohandquicken_lv > 0 && !sd->sc.getSCE(SC_TWOHANDQUICKEN)) {
				skill_castend_nodamage_id(&sd->bl, &sd->bl, KN_TWOHANDQUICKEN, twohandquicken_lv, gettick(), 0);
				// Use SP cost for the skill
				int sp_cost = skill_get_sp(KN_TWOHANDQUICKEN, twohandquicken_lv);
				status_zap(&sd->bl, 0, sp_cost);
			}
		}
	}
	...
	// Archer Class
	/////////////////////////////////////
	if (sd->class_ == MAPID_ARCHER || sd->class_ == MAPID_HUNTER || sd->class_ == MAPID_BARDDANCER) {
		//improve concentration
		int improveconcentration_lv = pc_checkskill(sd, AC_CONCENTRATION);
		if (improveconcentration_lv > 0 && !sd->sc.getSCE(SC_CONCENTRATE)) {
			skill_castend_nodamage_id(&sd->bl, &sd->bl, AC_CONCENTRATION, improveconcentration_lv, gettick(), 0);
			//use sp cost for use skill
			int sp_cost = skill_get_sp(AC_CONCENTRATION, improveconcentration_lv);
			status_zap(&sd->bl, 0, sp_cost);
		}
	}
	/////////////////////////////////////
	// Merchant Class
	/////////////////////////////////////
	if (sd->class_ == MAPID_BLACKSMITH) {
		//Adrenaline Rush
		if (sd && sd->status.weapon == W_MACE || sd && sd->status.weapon == W_2HMACE || sd && sd->status.weapon == W_1HAXE || sd && sd->status.weapon == W_2HAXE) {
			int adrenalinrush_lv = pc_checkskill(sd, BS_ADRENALINE);
			if (adrenalinrush_lv > 0 && !sd->sc.getSCE(SC_ADRENALINE)) {
				skill_castend_nodamage_id(&sd->bl, &sd->bl, BS_ADRENALINE, adrenalinrush_lv, gettick(), 0);
				//use sp cost for use skill
				int sp_cost = skill_get_sp(BS_ADRENALINE, adrenalinrush_lv);
				status_zap(&sd->bl, 0, sp_cost);
			}
		}
		//Weapon Perfection
		int weaponperfection_lv = pc_checkskill(sd, BS_WEAPONPERFECT);
		if (weaponperfection_lv > 0 && !sd->sc.getSCE(SC_WEAPONPERFECTION)) {
			skill_castend_nodamage_id(&sd->bl, &sd->bl, BS_WEAPONPERFECT, weaponperfection_lv, gettick(), 0);
			//use sp cost for use skill
			int sp_cost = skill_get_sp(BS_WEAPONPERFECT, weaponperfection_lv);
			status_zap(&sd->bl, 0, sp_cost);
		}
		//Power-Thrust
		if (sd && sd->status.weapon == W_MACE || sd && sd->status.weapon == W_2HMACE || sd && sd->status.weapon == W_1HAXE || sd && sd->status.weapon == W_2HAXE) {
			int powerthrust_lv = pc_checkskill(sd, BS_OVERTHRUST);
			if (powerthrust_lv > 0 && !sd->sc.getSCE(SC_OVERTHRUST)) {
				skill_castend_nodamage_id(&sd->bl, &sd->bl, BS_OVERTHRUST, powerthrust_lv, gettick(), 0);
				//use sp cost for use skill
				int sp_cost = skill_get_sp(BS_OVERTHRUST, powerthrust_lv);
				status_zap(&sd->bl, 0, sp_cost);
			}
		}
	}
...
static TIMER_FUNC(autobuff_timer) {
	struct map_session_data* sd = NULL;
	sd = map_id2sd(id);
	...
	if (sd->sc.option & OPTION_AUTOATTACK) {
		autoattack_buff_self(sd);
		if (sd->sc.option & OPTION_AUTOATTACK)
			add_timer(gettick() + battle_config.autobuff_interval, autobuff_timer, sd->bl.id, 0);
	}
	return 0;
}
...
static TIMER_FUNC(autoattack_timer) {
	...
	if (sd->sc.option & OPTION_AUTOATTACK) {
		...
		if (sd->sc.option & OPTION_AUTOATTACK)
			add_timer(gettick() + battle_config.autoattack_interval, autoattack_timer, sd->bl.id, (intptr_t)mob_id);
	}
	return 0;
}
ACMD_FUNC(autoattack) {
	...
	if (sd->sc.option & OPTION_AUTOATTACK) {
		...
		sd->autoattack_mob_id = 0;
		if (mob_id) {
			sd->autoattack_mob_id = mob_id;
		}
		sd->sc.option |= OPTION_AUTOATTACK;
		sd->autoattack_need_rest = false;
		add_timer(gettick() + 800, autoattack_timer, sd->bl.id, (intptr_t)mob_id);
		add_timer(gettick() + 800, autobuff_timer, sd->bl.id, 0);


		clif_displaymessage(sd->fd, msg_txt(sd, 2506));
	}
	clif_changeoption(&sd->bl);
	return 0;
}
...
ACMD_FUNC(whosell) {
	...
	if (sscanf(message, "+%d %d[%d]", &refine, &item_id, &card_id) == 3) {
		...
		if ((item_data = item_db.searchname(item_name)) == NULL) {
			clif_displaymessage(fd, msg_txt(sd, 2510)); // Not found item with this name
			return -1;
		}
		item_id = item_data->nameid;
	}
	...
	if (s_type & 2 && ((item_data = item_db.find(card_id)) == NULL || item_data->type != IT_CARD)) {
		clif_displaymessage(fd, msg_txt(sd, 2511)); // Not found a card with than ID
		return -1;
	}
	//check item
	if (s_type & 1 && (item_data = item_db.find(item_id)) == NULL) {
		clif_displaymessage(fd, msg_txt(sd, 2512)); // Not found an item with than ID
		return -1;
	}
	//check refine
	if (s_type & 4) {
		if (refine < 0 || refine>10) {
			clif_displaymessage(fd, msg_txt(sd, 2513)); // Refine out of bounds: 0 - 10
			return -1;
		}
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		if (pl_sd->vender_id) {	 // check if player is vending
			for (j = 0; j < pl_sd->vend_num; j++) {
				...
				sprintf(atcmd_output, msg_txt(sd, 2514)
					...
					, pl_sd->message); //%s - Price: %uz - Amount: %d - %s %d,%d - %s
	...
	mapit_free(iter);
	if (count > 0) {
		sprintf(atcmd_output, msg_txt(sd, 2515), count); //Found %d entries.
		clif_displaymessage(fd, atcmd_output);
	}
	else
		clif_displaymessage(fd, msg_txt(sd, 2516)); // Nobody is selling it now.
	return 0;
}
