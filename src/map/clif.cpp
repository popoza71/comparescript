...
void clif_barter_extended_open(map_session_data& sd, struct npc_data& nd) {
	...
	if(barter->is_craft){
		...
		clif_broadcast(&sd.bl, output, strlen(output) + 1, BC_BLUE, SELF);
		clif_messagecolor(&sd.bl, color_table[COLOR_DEFAULT], msg_txt(NULL, 3002), false, SELF);
		clif_messagecolor(&sd.bl, color_table[COLOR_DEFAULT], msg_txt(NULL, 3003), false, SELF);
		...
		clif_messagecolor(&sd.bl, color_table[COLOR_DEFAULT], msg_txt(NULL, 3008), false, SELF);
	}
...
void clif_parse_barter_extended_buy(int fd, map_session_data* sd) {
	...
	if(barter->is_craft && entries > 1){
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msg_txt(NULL, 3009), false, SELF);
		return;
	}

	if(barter->is_craft && p->list[0].amount > 1){
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msg_txt(NULL, 3013), false, SELF);
		return;
	}
