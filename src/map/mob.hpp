...
int mob_getdroprate(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, mob_data * md = nullptr); //gum split


//puppy @mi
struct DropRateResult {
	int normal_drop_rate;
	int bonus_drop_rate;
	int total_drop_rate;
};

DropRateResult mob_getdroprate_extended(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, mob_data* md = nullptr);
//puppy @mi
