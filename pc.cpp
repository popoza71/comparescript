...
void pc_bonus2(map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_SUBELE: // bonus2 bSubEle,e,x;
		PC_BONUS_CHK_ELEMENT(type2,SP_SUBELE); //add max capping
		if (sd->state.lr_flag != 2) {
			sd->indexed_bonus.subele_script[type2] += val;
			//add max capping
			if (sd->indexed_bonus.subele_script[type2] > battle_config.sub_ele_capping)
				sd->indexed_bonus.subele_script[type2] = battle_config.sub_ele_capping;
		}
		break;
...
static void pc_calcexp(map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	// PUPPY EXTRA BUFF
	if (sd->sc.getSCE(SC_BONUSEXP)) {
		bonus += sd->sc.getSCE(SC_BONUSEXP)->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.getSCE(SC_BONUSEXP)->val1 / battle_config.vip_bm_increase);
	}
	if (sd->sc.getSCE(SC_BONUSEXP2)) {
		bonus += sd->sc.getSCE(SC_BONUSEXP2)->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.getSCE(SC_BONUSEXP2)->val1 / battle_config.vip_bm_increase);
	}
	...
	// PUPPY EXTRA BUFF
	if (sd->sc.getSCE(SC_BONUSJOB))
		bonus += sd->sc.getSCE(SC_BONUSJOB)->val1;
	if (sd->sc.getSCE(SC_BONUSJOB2))
		bonus += sd->sc.getSCE(SC_BONUSJOB2)->val1;
	...
	if(percent && sd->status.base_level >= battle_config.rank_title_min_lv){
		...
		if(rank_exp){
			...
			if(next_rank){
				...
				for(int z=0;z<sort_rank_title_list.size();z++){
					std::shared_ptr<s_rank_title> rix = rank_title_db.find(sort_rank_title_list[z]);
					if(rank_title < rix->title_id && current_rank_exp >= rix->exp){
						rank_index = rix->title_id;
					}
				}
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
	...
	if( battle_config.party_bonus_system_enable && battle_config.party_bonus_no_die_trade_chat && sd->status.party_id ){
	...
	if( !battle_config.pk_mode && mapdata->getMapFlag(MF_PVP) && !mapdata->getMapFlag(MF_PVP_NOCALCRANK) ) {
		...
		//if( sd->pvp_point < 0 ) {
		if (sd->pvp_point < 0 && !mapdata->getMapFlag(MF_MAPMVP)) { //mapmvp
...
bool pc_revive_item(map_session_data *sd) {
	...
	if (map_getmapflag(sd->bl.m, MF_NOTOKEN)) {
		clif_showscript(&sd->bl, msg_txt(NULL, 2508), SELF);
		return false;
	}
	//pp MF_NOTOKEN
...
bool pc_equipitem(map_session_data *sd,short n,int req_pos,bool equipswitch)
{
	...
	if( equipswitch ){
		...
		// puppy show shadow weapon
		// [ Shadow Weapon costume sprite update ]
		if (id && id->equip == EQP_SHADOW_WEAPON && battle_config.shadow_weapon_view) {
			t_itemid rhand = 0, lhand = 0;
			clif_get_weapon_view(sd, &rhand, &lhand);
			clif_changelook(&sd->bl, LOOK_WEAPON, rhand);
			//ShowDebug(">> Shadow Weapon EQUIPPED: rhand = %d (%s)\n",
			//	rhand, itemdb_exists(rhand) ? itemdb_search(rhand)->name : "UNKNOWN");
		}
		// puppy show shadow weapon
...
static void pc_unequipitem_sub(map_session_data *sd, int n, int flag) {
	...
	// puppy show shadow weapon
	if (sd->inventory_data[n] && sd->inventory_data[n]->equip & EQP_SHADOW_WEAPON) {
		t_itemid rhand = 0, lhand = 0;
		clif_get_weapon_view(sd, &rhand, &lhand);
		clif_changelook(&sd->bl, LOOK_WEAPON, rhand);
		//ShowDebug(">> Shadow Weapon unequipped. Updated rhand: %d (%s)\n", rhand,
		//	itemdb_exists(rhand) ? itemdb_search(rhand)->name : "UNKNOWN");
	}
	// puppy show shadow weapon
...
void pc_set_costume_view(map_session_data *sd) {
	...
	//sd->status.head_bottom = sd->status.head_mid = sd->status.head_top = sd->status.robe = 0; //puppy
	sd->status.head_bottom = sd->status.head_mid = sd->status.head_top = sd->status.robe = sd->status.cweapon = 0; //puppy
...
void pc_collection_update(struct s_storage *stor, map_session_data &sd) {
	...
	if ((sd.state.collection_flag&PCCOLLECTION_LOAD && sd.collection_list.size() > 0) || sd.state.collection_flag&PCCOLLECTION_RECAL) {
		sprintf(output, msg_txt(&sd,(sd.state.collection_flag&PCCOLLECTION_LOAD ? 1750 : 1751)), sd.collection_list.size());
		clif_messagecolor(&sd.bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		status_calc_pc(&sd, SCO_FORCE);
	}
...
void do_init_pc(void) {
	...
	add_timer_func_list(pc_goldpc_update, "pc_goldpc_update"); //goldpc
